...
import { MotionEnter, MotionReveal } from "@/components/motion"
...
export function KfteOsPageContent() {
  const instagram = footerContent.social.find((s) => s.label === "Instagram")

  return (
    <div className="flex min-h-[calc(100vh-96px)] flex-col bg-surface px-8 py-10 xl:px-12 2xl:px-16">
      <MotionReveal>
      ...
      <div className="mt-auto flex flex-col items-start gap-3 pb-8">
        {portalLinks.map((link, index) => (
          <MotionEnter key={link.href} delay={0.08 + index * 0.06}>
            <Link
              href={link.href}
