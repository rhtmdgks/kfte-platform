...
export default async function EditEventArchivePage({ params }: PageProps) {
  ...
  const { data: post } = await supabase
    .from("content_posts").select("*").eq("id", id).eq("content_type", "event_archive").single()
  ...
  return (
    <div className="flex flex-col">
      <header className="flex h-14 items-center gap-4 border-b px-6">
        ...
        <h1 className="text-lg font-semibold text-[#002065]">행사 아카이브 수정</h1>
      </header>
      <main className="flex-1 p-6">
        <ContentPostForm post={post} contentType="event_archive" action={updatePost.bind(null, id)} />
      </main>
    </div>
  )
}
