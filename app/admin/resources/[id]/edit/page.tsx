...
export default async function EditResourcePage({ params }: PageProps) {
  ...
  const { data: post } = await supabase
    .from("content_posts").select("*").eq("id", id).eq("content_type", "resource").single()
  ...
  return (
    <div className="flex flex-col">
      <header className="flex h-14 items-center gap-4 border-b px-6">
        ...
        <h1 className="text-lg font-semibold text-[#002065]">자료 수정</h1>
      </header>
      <main className="flex-1 p-6">
        <ContentPostForm post={post} contentType="resource" action={updatePost.bind(null, id)} />
      </main>
    </div>
  )
}
